...
namespace Landis.Biomass.Succession
{
        ...
        : Biomass.ICalculator
    {
                                 ...
                                 int prevYearSiteMortality)
        {
            ...
            if (PlugIn.CalibrateMode && Model.Core.CurrentTime > 0)
            {
                UI.WriteLine("Yr={0}. Calculate Delta Biomass...", (Model.Core.CurrentTime+SubYear));
                UI.WriteLine("Yr={0}.    Spp={1}, Age={2}.", (Model.Core.CurrentTime+SubYear), cohort.Species.Name, cohort.Age);
                UI.WriteLine("Yr={0}.    ANPPact={1:0.0}, Mtotal={2:0.0}, litter={3:0.00}.", (Model.Core.CurrentTime+SubYear), actualANPP, totalMortality, totalLitter);
                UI.WriteLine("Yr={0}.    DeltaB={1:0.0}, CohortB={2}, Bsite={3}", (Model.Core.CurrentTime+SubYear), deltaBiomass, cohort.Biomass, (int) siteBiomass);
            }
                                         ...
                                         int prevYearSiteMortality)
        {
            ...
            B_PM = indexLightC;
            UI.WriteLine("indexC={0:0.00}, lightIndexC={1:0.00}, OldSchool={2:0.00}.", indexC, indexLightC, indexOldSchool);
            ...
            if(PlugIn.CalibrateMode && Model.Core.CurrentTime > 0)
            {
                ...
                UI.WriteLine("Yr={0}.     Spp={1}, Age={2}.", (Model.Core.CurrentTime+SubYear), cohort.Species.Name, cohort.Age);
                UI.WriteLine("Yr={0}.     MaxANPP={1}, MaxB={2:0}, Bsite={3}, Bcohort={4:0.0}.", (Model.Core.CurrentTime+SubYear), maxANPP, maxBiomass, (int) siteBiomass, cohort.Biomass);
                UI.WriteLine("Yr={0}.     B_PM={1:0.0}, B_AP={2:0.0}, actualANPP={3:0.0}, capacityReduction={4:0.0}.", (Model.Core.CurrentTime+SubYear), B_PM, B_AP, actualANPP, capacityReduction);
                UI.WriteLine("Yr={0}.     CanopyLightExtinction = {1:0.00}, LightTransmittance = {2:0.00}, LAIactual={3:0.0}.", (Model.Core.CurrentTime + SubYear), CanopyLightExtinction, B_PM, LAIactual);
            }
        ...
        private static double CalculateCompetition(ActiveSite site, ICohort cohort)
        {
            ...
            //UI.WriteLine("Competition:  spp={0}, age={1}, CMultiplier={2:0}, CMultTotal={3:0}.", cohort.Species.Name, cohort.Age, CMultiplier, CMultTotal);
            ...
            foreach (ISpeciesCohorts speciesCohorts in SiteVars.Cohorts[site])
            {
                foreach (ICohort xcohort in speciesCohorts)
                {
                    if (xcohort.Age+1 != cohort.Age || xcohort.Species.Index != cohort.Species.Index)
                    {
                        ...
                        //UI.WriteLine("Competition:  spp={0}, age={1}, CMultiplier={2:0}, CMultTotal={3:0}.", xcohort.Species.Name, xcohort.Age, tempMultiplier, CMultTotal);
                    }
                }
            }
            ...
            //UI.WriteLine("Competition:  spp={0}, age={1}, CMultiplier={2:0}, CMultTotal={3:0}, CI={4:0.00}.", cohort.Species.Name, cohort.Age, CMultiplier, CMultTotal, Cfraction);
