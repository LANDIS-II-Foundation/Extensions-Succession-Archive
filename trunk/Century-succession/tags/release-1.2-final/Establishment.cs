...
namespace Landis.Extension.Succession.Century
{
    public class Establishment
    {
        ...
        public static void Initialize()
        {
            ...
            UI.WriteLine("   Opening a Century log file \"{0}\" ...", logFileName);
            try {
        ...
        public static Species.AuxParm<Ecoregions.AuxParm<double>>  GenerateNewEstablishProbabilities(int years)
        {
            ...
            EstablishProbability = CreateSpeciesEcoregionParm<double>(Model.Core.Species, Model.Core.Ecoregions);

            double[,] avgTempMultiplier = new double[Model.Core.Species.Count, Model.Core.Ecoregions.Count];
            double[,] avgSoilMultiplier = new double[Model.Core.Species.Count, Model.Core.Ecoregions.Count];
            double[,] avgMinJanTempMultiplier = new double[Model.Core.Species.Count, Model.Core.Ecoregions.Count];
            ...
            foreach(IEcoregion ecoregion in Model.Core.Ecoregions) 
            {
                foreach(ISpecies species in Model.Core.Species)
                {
                    ...
                    if(Model.Core.CurrentTime > 0 && EcoregionData.ActiveSiteCount[ecoregion] > 0)
                    {
                        ...
                        log.Write("{0}, {1}, {2},", Model.Core.CurrentTime, ecoregion.Name, species.Name);
...

        {
            ...
            if (growDays < 2.0)
            {
                UI.WriteLine("Begin Grow = {0}, End Grow = {1}", weather.BeginGrowing, weather.EndGrowing);
            ...
            //UI.WriteLine("SppMaxDr={0:0.00}, growDays={1:0.0}, dryDays={2:0.0}.", sppAllowableDrought, growDays, dryDays);
        ...
        private static Species.AuxParm<Ecoregions.AuxParm<double>> CreateSpeciesEcoregionParm<T>(Species.IDataset speciesDataset, Ecoregions.IDataset ecoregionDataset)
        {
