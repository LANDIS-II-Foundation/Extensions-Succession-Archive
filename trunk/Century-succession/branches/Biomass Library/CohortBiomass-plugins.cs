...
namespace Landis.Extension.Succession.Century
{
        ...
        : Landis.Library.LeafBiomassCohorts.ICalculator
    {
        ...
        public float[] ComputeChange(ICohort cohort, ActiveSite site)
        {
            ...
            //PlugIn.ModelCore.UI.WriteLine("Mortality.Month={0:0.0}, Leafbiomass={1:0.000}, MortAge={2:0.00}, MortGrow={3:0.00}", Century.Month, cohort.LeafBiomass, mortalityAge[1], mortalityGrowth[1]);
            ...
            if (Century.Month == 6)  //July = 6
            {
                ...
                if (defoliation > 0.0)
                {
                    ...
                    //defoliatedLeafBiomass = (cohort.LeafBiomass + actualANPP[1] - totalMortality[1]) * defoliation;  //TESTING
                    ...
                    //PlugIn.ModelCore.UI.WriteLine("Defoliation.Defoliatedleafbiomass={0:0.000}, Leafbiomass={1:0.000}, Month={2:0.0}, Defoliation={3:0.00}", defoliatedLeafBiomass, cohort.LeafBiomass, Century.Month, defoliation);
            ...
            {
                PlugIn.ModelCore.UI.WriteLine("Warning: LEAF Mortality exceeds cohort biomass. Mortality={0:0.000}, B={1:0.000}", (totalMortality[1] + defoliatedLeafBiomass), cohort.LeafBiomass);
            ...
            //PlugIn.ModelCore.UI.WriteLine("ChangeANPP.month={0:0.0}, deltaLeaf={1:0.000}, actualANPP={2:0.000}, defoliatedleafbiomass={3:0.000},totalLeafMortality={4:0.00}", Century.Month, deltaLeaf, actualANPP[1], defoliatedLeafBiomass, totalMortality[1]);

            float[] deltas = new float[2] { deltaWood, deltaLeaf };

            if(totalMortality[1] + defoliatedLeafBiomass > cohort.LeafBiomass)
                PlugIn.ModelCore.UI.WriteLine("Warning: Leaf Mortality exceeds cohort leaf biomass. M={0:0.0}, B={1:0.0}", (totalMortality[0] + totalMortality[1]), (cohort.WoodBiomass + cohort.LeafBiomass));
        ...
        private static double calculateWater_Limit(ActiveSite site, IEcoregion ecoregion, ISpecies species)
        {
            ...
            {   //       Trees are allowed to access the whole soil profile -rm 2/97
                ...
                //PlugIn.ModelCore.UI.WriteLine("Yr={0},Mo={1}.     AvailH20={2:0.0}, MonthPPT={3:0.0}, PET={4:0.0}.", PlugIn.ModelCore.CurrentTime, month, SiteVars.AvailableWater[site],tmoist,pet);
            }
            else pptprd = 0.01;
            ...
            //PlugIn.ModelCore.UI.WriteLine("Yr={0},Mo={1}.     AvailH20={2:0.0}, MonthlyPPT={3:0.0}, PET={4:0.0}, PPTPRD={5:0.0}, Limit={6:0.0}.", PlugIn.ModelCore.CurrentTime, month, SiteVars.AvailableWater[site],tmoist, pet,pptprd,pprdwc);
