...
namespace Landis.Biomass.Succession
{
    ...
    public class LivingBiomass
    {
        ...
        {
            int youngBiomass;
            double totalBiomass = Cohorts.ComputeBiomass(siteCohorts, out youngBiomass);
        ...
        /// <summary>
        /// Computes the leaf biomass at a site.  The leaf biomass is the total
        /// of all the site's cohorts minus young cohorts and minus what is lost
        /// due to defoliation.  
        /// </summary>
        /*public static double ActualSiteLeafBiomass(SiteCohorts    siteCohorts,
                                               ActiveSite     site)
        {
            double cohortLeafBiomass = 0.0;
            double totalLeafBiomass = 0.0;
            foreach (ISpeciesCohorts speciesCohorts in siteCohorts) {    
                foreach(ICohort cohort in speciesCohorts)
                {
                    if(cohort.Age > 1)
                        cohortLeafBiomass = cohort.ComputeNonWoodyBiomass(site);
                    totalLeafBiomass += cohortLeafBiomass;
                }
            }
            //Potential defoliation.
            if (SiteVars.InsectDefoliation != null)
                totalLeafBiomass *= (double) SiteVars.InsectDefoliation[site] / 100.0;

            return totalLeafBiomass;
        }*/
        //---------------------------------------------------------------------
        ...
        public static byte ComputeShade(ActiveSite site)
        {
            ...
            //double B_leaf = ActualSiteLeafBiomass(siteCohorts, site);

            //  Relative living biomass (ratio of actual to maximum site
            //  biomass).
            double B_AM = B_ACT / B_MAX[ecoregion];
            
            // Adjust for loss of foliage caused by defoliation:
            if (SiteVars.InsectDefoliation != null)
                B_AM *= 1.0 - ((double) SiteVars.InsectDefoliation[site] / 100.0);
