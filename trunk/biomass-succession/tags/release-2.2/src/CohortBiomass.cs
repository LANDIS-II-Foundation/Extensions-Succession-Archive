...
namespace Landis.Biomass.Succession
{
        ...
        : Biomass.ICalculator
    {
                                         ...
                                         int        prevYearSiteMortality)
        {
            ...
            //double indexC = 0.0;  // competition index
            //Dictionary<int, double> cohortDict;
            //if (CohortCompetition.TryGetValue(cohort.Species.Index, out cohortDict))
            //    cohortDict.TryGetValue(cohort.Age-1, out indexC);
            ...
            if((indexC <= 0.0 && cohortBiomass > 0) || indexC > 1.0)
            {
                ...
                //foreach (KeyValuePair<int, Dictionary<int, double>> kvp in CohortCompetition)
                //{
                //    foreach(KeyValuePair<int, double> kvp2 in kvp.Value)
                //    {
                //        UI.WriteLine("    CI Dictionary:  Species={0}, Age={1}, CI={2}.", Model.Core.Species[kvp.Key].Name, kvp2.Key, kvp2.Value);
                //    }
                //}
...
/*        public static void CalculateCompetition(ActiveSite site)
        {

            //Iterate through the first time, assigning each cohort an un-normalized C multiplier
            CohortBiomass.CohortCompetition = new Dictionary<int, Dictionary<int, double>>();
        ...
        private static double CalculateCompetition(ActiveSite site, ICohort cohort)
        {
            ...
            //Dictionary<int,double> newEntry = new Dictionary<int,double>();
            //newEntry.Add(cohort.Age-1, CMultiplier);
            //CohortCompetition.Add(cohort.Species.Index, newEntry);
